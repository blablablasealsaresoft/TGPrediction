"""
Add all wallets from unique_wallets_list.txt to the database for copy trading
"""
...
async def add_all_wallets():
    """Add all wallets from unique_wallets_list.txt"""
    db = DatabaseManager()
    
    # Read wallets from file
    wallets_to_add = []
    
    print("\n📖 Reading unique_wallets_list.txt...")
    ...
    for i, wallet in enumerate(new_wallets, 1):
        try:
            ...
            if failed_count <= 5:  # Only show first 5 errors
                print(f"   ⚠️ Failed to add {wallet[:8]}...: {e}")
...
if __name__ == "__main__":
    print("\n🚀 BULK WALLET IMPORT")
    print("=" * 60)
    print("This will add all wallets from unique_wallets_list.txt")
    print("to the database for copy trading monitoring.")
    print("=" * 60)
